...
class WpDashboardController extends Controller
{
    ...
    public function bayarSptpd(Request $request, $id)
    {
        $request->validate([
            'metode_bayar' => 'required|in:transfer,tunai,va',
            'bukti_bayar' => 'nullable|file|mimes:pdf,jpg,jpeg,png|max:2048',
            'keterangan' => 'nullable|string|max:500'
        ]);

        $wp = Auth::user()->wp;
        $sptpd = Sptpd::where('id', $id)
                     ->where('subjek_pajak_id', $wp->subjek_pajak_id)
                     ->where('status', '!=', 'Lunas')
                     ->firstOrFail();
        ...
        $totalBayar = $sptpd->pajak_terutang + $sptpd->denda + $sptpd->bunga + $sptpd->kenaikan - $sptpd->kompensasi - $sptpd->setoran;
        ...
        $sptpd->update([
            ...
            'keterangan_bayar' => $request->keterangan,
            'tanggal_bayar' => now(),
        ]);

        return response()->json([
            'success' => true,
            'message' => 'Pembayaran berhasil dicatat. Status SPTPD diubah menjadi Lunas.',
            'data' => $sptpd
        ]);
    }
    ...
    public function buktiBayar($id)
    {
        ...
        $pdf = app('dompdf.wrapper');
        $pdf->loadView('wp.bukti_bayar_pdf', compact('sptpd'));
        
        return $pdf->download('Bukti-Bayar-' . $sptpd->nomor_sptpd . '.pdf');
    }
}
