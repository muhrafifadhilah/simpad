...
<head>
    <meta charset="utf-8">
    <title>Bukti Bayar - {{ $sptpd->nomor_sptpd }}</title>
    <style>
        body { 
...
<body>
    ...
    <div class="bukti-info">
        ...
        <div class="info-row">
            ...
            <div class="info-colon">:</div>
            <div class="info-value fw-bold">{{ \Carbon\Carbon::parse($sptpd->tanggal_bayar)->format('d F Y H:i') }}</div>
        </div>
        <div class="info-row">
            <div class="info-label">Metode Pembayaran</div>
            <div class="info-colon">:</div>
            <div class="info-value">{{ ucfirst($sptpd->metode_bayar) }}</div>
        </div>
        <div class="info-row">
    ...
    <div class="wp-data">
        ...
        <table class="wp-table">
            <tr>
                ...
                <td>{{ $sptpd->objekPajak->jenis_pajak ?? 'Pajak Umum' }}</td>
            </tr>
            <tr>
    ...
    <div class="payment-details">
        ...
        <table class="calc-table">
            <tr>
                ...
                <td class="calc-value">Rp {{ number_format($sptpd->dasar, 0, ',', '.') }}</td>
            </tr>
            <tr>
                <td class="calc-label">Tarif Pajak</td>
                <td class="calc-value">{{ $sptpd->tarif }}%</td>
            </tr>
            <tr>
                <td class="calc-label">Pajak Terutang</td>
                <td class="calc-value">Rp {{ number_format($sptpd->pajak_terutang, 0, ',', '.') }}</td>
            </tr>
            @if($sptpd->denda > 0)
            <tr>
                <td class="calc-label">Denda Keterlambatan</td>
                <td class="calc-value">Rp {{ number_format($sptpd->denda, 0, ',', '.') }}</td>
            </tr>
            @endif
            @if($sptpd->bunga > 0)
            <tr>
                <td class="calc-label">Bunga</td>
                <td class="calc-value">Rp {{ number_format($sptpd->bunga, 0, ',', '.') }}</td>
            </tr>
            @endif
            @if($sptpd->kenaikan > 0)
            <tr>
                <td class="calc-label">Kenaikan</td>
                <td class="calc-value">Rp {{ number_format($sptpd->kenaikan, 0, ',', '.') }}</td>
            </tr>
            @endif
            @if($sptpd->kompensasi > 0)
            <tr>
                <td class="calc-label">Kompensasi</td>
                <td class="calc-value">- Rp {{ number_format($sptpd->kompensasi, 0, ',', '.') }}</td>
            </tr>
            @endif
            <tr class="total-row">
                <td class="calc-label">TOTAL YANG DIBAYAR</td>
                <td class="calc-value">
                    Rp {{ number_format($sptpd->setoran, 0, ',', '.') }}
                </td>
            </tr>
        </table>
    </div>
    ...
    @if($sptpd->keterangan_bayar)
    <div class="payment-info">
        <h3>KETERANGAN PEMBAYARAN</h3>
        <p>{{ $sptpd->keterangan_bayar }}</p>
    </div>
    @endif
    ...
    <div class="footer">
        <div class="signature-area">
            <div class="signature-left">
                ...
                <p>Tanggal: {{ \Carbon\Carbon::parse($sptpd->tanggal_bayar)->format('d F Y') }}</p>
            </div>
